...
export const TransferForm = () => {
    ...
    return (
        <div className="p-6 bg-[var(--color-deep-purple)] rounded-xl shadow-lg border border-[var(--color-surface-border)] max-w-2xl mx-auto animate-fadeIn">
            <h2 className="text-2xl font-bold text-[var(--color-fuchsia-vivid)] mb-6">Nueva Transferencia</h2>
            <form onSubmit={onSubmit} className="flex flex-col gap-4">
                <div>
                    <label className="text-[var(--color-text-dark-secondary)] mb-2 block font-medium">Mi Cuenta (Origen)</label>
                    <input
                        type="text"
                        disabled
                        className="w-full p-3 bg-[var(--color-space-bg)] text-[var(--color-text-dark-tertiary)] rounded-lg border border-[var(--color-surface-border)] outline-none opacity-70 cursor-not-allowed font-mono"
                        value={user?.username || user?.email || 'Cuenta vinculada (Automático)'}
                    />
                </div>
                <div>
