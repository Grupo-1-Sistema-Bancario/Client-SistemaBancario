...
const NAV = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        id: "nuevoCliente", to: "/dashboard/NewClients", label: "Nuevo Cliente", badge: null, icon: (
          <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-5">
          {NAV.map(group => (
            <div key={group.section}>
              <p className="px-3 mb-1.5 text-[9px] font-semibold tracking-[0.2em] uppercase text-purple-500/70">
                {group.section}
              </p>
              <ul className="space-y-0.5">
                {group.items.map(item => {
                  // 2. DETECCIÓN DE RUTA ACTIVA: Comparamos la URL actual con item.to
                  const isActive = location.pathname === item.to;

                  return (
                    <li key={item.id}>
                      <Link
                        to={item.to || "#"} // Usamos Link de react-router-dom
                        className={`
